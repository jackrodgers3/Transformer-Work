...
for i in range(len(data)):
    try:
        unique_tokens.index(data[i])
    except ValueError:
        unique_tokens.append(data[i])
unique_tokens = sorted(unique_tokens)
EX_TEXT = 'I think I want food.'.lower()
EX_TEXT = simple_separator(EX_TEXT)

stoi = {ch: i for i, ch in enumerate(unique_tokens)}
itos = {i: ch for i, ch in enumerate(unique_tokens)}
encode = lambda s: [stoi[c] for c in s]
decode = lambda l: ''.join([itos[i] for i in l])
print(encode(EX_TEXT))
...
data_nn = torch.tensor(encode(data), dtype=torch.long)
n = int(0.9*len(data_nn))
train_data = data_nn[:n]
valid_data = data_nn[n:]
